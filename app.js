...
function generateAiResponse(message) {
    ...
    if (message.includes("анализ") || message.includes("рекомендации") || message.includes("что ты можешь посоветовать")) {
    ...
    // 2. СТРАТЕГИИ ИЗМЕНЕНИЯ ВЕСА
        ...
        response += `* **Питание:** Ваш текущий калораж ${data.food} ккал. Рекомендуется снизить его до 1500-1800 ккал (зависит от вашей базы) и увеличить потребление белка.\n`;
        response += `* **Активность:** Ваши шаги ${data.steps}. Стремитесь к 10,000. Включите силовые тренировки (${data.workout} мин.) для сохранения мышечной массы.`;
        ...
        response += `* **Питание:** Ваш текущий калораж ${data.food} ккал. Повысьте его на 300-500 ккал, с акцентом на сложные углеводы и белок (2.2 г на 1 кг веса).\n`;
        response += `* **Тренировки:** Фокусируйтесь на прогрессирующей силовой нагрузке. Ваши тренировки: ${data.workout} мин. Они должны быть регулярными (3-4 раза в неделю).`;
        
    // 3. КОНКРЕТНЫЕ ТРЕКЕРЫ (Более контекстные ответы)
    } else if (message.includes("сколько стаканов воды")) {
        response = `💧 **Гидратация:** Вы выпили ${data.water} стаканов. Это меньше 8-ми рекомендуемых стаканов (около 2 литров). Недостаток воды влияет на вашу энергию и метаболизм.`;
    } else if (message.includes("сколько шагов") || message.includes("норма шагов")) {
    ...
    } else if (message.includes("сколько сна") || message.includes("качество сна")) {
    ...
    } else if (message.includes("питание") || message.includes("калорий")) {
    ...
    } else if (message.includes("тренировки") || message.includes("активность")) {
    ...
    // 4. БАЗОВЫЕ ОТВЕТЫ
        ...
        response = "Привет! Я твой SMART Health AI. Спросите **анализ трекера** или **стратегию похудения/набора веса**.";
    } else {
        response = "Я не понял ваш запрос. Я могу ответить на сложные вопросы (про похудение, набор веса, анализ трекера) или дать информацию о воде, сне, питании и шагах.";
    }

    return response;
}

// Функцию analyzeCurrentDataForChat менять не нужно, она отлично работает для комплексного анализа!
